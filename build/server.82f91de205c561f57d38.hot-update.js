...
let BillService = class BillService {
  ...
  async balance(userID, {
    ...
    const res = await queryBuilder.select('"bill"."id", sum("bill"."value") as balance').where({
      user: userID,
      createdAt: {
        $gte: (0, date_1.day)(date).startOf("month").toDate(),
        $lte: (0, date_1.day)(date).endOf("month").toDate()
      }
    }).groupBy('"bill"."id", "balance"').getKnexQuery();
    console.log(res);
    return res;
  }
...
/******/ 	(() => {
/******/ 		__webpack_require__.h = () => ("34e64d00510c7974add1")
/******/ 	})();
/******/ 	
/******/ }
;
//# sourceMappingURL=server.82f91de205c561f57d38.hot-update.js.map
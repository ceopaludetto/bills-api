...
let BillService = class BillService {
  ...
  async balance(userID, {
    ...
    const res = await queryBuilder.select('sum("bill"."value") as balance').where({
      user: userID,
      createdAt: {
        $gte: (0, date_1.day)(date).startOf("month").toDate(),
        $lte: (0, date_1.day)(date).endOf("month").toDate()
      }
    }).groupBy('"bill"."id", "bill"."value"').getResult();
    console.log(res);
    return res;
  }
...
/******/ 	(() => {
/******/ 		__webpack_require__.h = () => ("9adcf7757af3df544f4b")
/******/ 	})();
/******/ 	
/******/ }
;
//# sourceMappingURL=server.095dfbdef0d5e106122c.hot-update.js.map